...
def init_temporal_adapter(device, cfg_model):
    adapter_name = cfg_model.get("temporal_adapter", "identity")
    if adapter_name == "samba":
        adapter = SambaTemporalAdapter(
            dim=cfg_model.get("pred_embed_dim", 384),
            depth=cfg_model.get("temporal_depth", 2),
            block_cfg=cfg_model.get("temporal_block_cfg", {}),
        )
    else:
        adapter = IdentityTemporalAdapter()
